...
function PlaylistSong(props) {
  ...
  return (
    <li
      ...
      onClick={() => {
        props.player.current.src = require(`../audio/${props.title}.mp3`);
        props.playOrPauseSong();
        props.songTitleRef.current.textContent = props.title;
      }}
    >
      <audio
        src={require(`../audio/${props.title}.mp3`)}
        ref={audioRef}
        onLoadedData={() =>
          (durationRef.current.textContent = convertToTime(
