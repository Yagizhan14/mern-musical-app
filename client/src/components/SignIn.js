...
const SignIn = (props) => {
    ...
    const submitHandler = e => {
        ...
        document.querySelector('.sign-in-form button').setAttribute('disabled',true)
        const credentials = {
            email:email.trim(),
        ...
        setTimeout(() => {
            ...
            document.querySelector('.sign-in-form button') && document.querySelector('.sign-in-form button').removeAttribute('disabled')
            if(isAuthenticated) {  
                clearErrors()
                props.history.push('/')
            }
    ...
    return (
        <div className="sign-in"> 
            ...
            <form className="sign-in-form" onSubmit={submitHandler}>
                ...
                <button type="submit" style={buttonLoading ? {backgroundColor:'grey',cursor:'not-allowed'} : {backgroundColor:'rgb(25,111,61)',cursor:'pointer'}}><span style={buttonLoading ? {display:'block'} : null}></span>{buttonLoading ? null : "Sign In"}</button>
            </form>
        </div>
    )
}
