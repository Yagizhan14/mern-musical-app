...
const Search = () => {
        ...
        switch(action.type){
            case 'PLAYING_HANDLER':
                const newState = state.map(song => {
                    if(song.title !== action.payload) return {title:song.title,playing:false}
                    else{
    ...
    const filterKeyUp = () => {
        ...
        listItemsTitles.forEach(item => {
            if(item.textContent.toLowerCase().indexOf(term) !== -1 || filterInput === '') {
    ...
    return (
        <div className="search-page">
            <div className="search-input-field">
                <input className="search-input" type="text" value={filterInput} onChange={filterHandler} onKeyUp={filterKeyUp} placeholder="Type the name of the song or the artist you want to filter"/>
            </div>
                ...
                {songs.map((song,index) => {
                    return(
                        <li className="song" key={index}>
                            <Song title={song.title} playing={song.playing} playingHandler={dispatchSongs}/>
                        </li>
                    )
                })}
            </ul>
